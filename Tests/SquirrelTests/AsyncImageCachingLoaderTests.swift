...
final class AsyncImageCachingLoaderTests: XCTestCase {

    // MARK: - Lifecycle
    
    override func setUpWithError() throws {
        sut = AsyncImageCachingLoader(
            cache: AsyncImageCachingLoaderTests.cacheSpy,
            load: AsyncImageCachingLoaderTests.loadMockSpy
        )
    }
    ...
    var sut: AsyncImageCachingLoader!
    ...
    static let loadMock: (URL) async throws -> (Data, URLResponse) = { url in
        ...
        case TestURLString.forbiddenURL: throw Loader.LoaderError.invalidResponse(httpStatusCode: 403)
        default: return try await URLSession.shared.data(from: url)
        }
    }
    
    // MARK: Methods
    
    func test_load_validURL_cachesAndReturnsPlatformImageFromRemote() async throws {
        // Given
        ...
        case .failure(let error):
            XCTFail(
                """
                Expected to receive a platform image from a valid URL but
                got \(String(describing: error)) instead.
                """)
        case nil:
    ...
    func test_load_validURLASecondTime_returnsDataFromCache() async throws {
        ...
        case .failure(let error):
            XCTFail(
                """
                Expected to receive a platform image for a valid URL but got
    ...
    func test_load_nilURL_returnsExpectedError() async throws {
        // Given
        let url: URL? = nil
        ...
        // Then
        let expectedNumberOfCacheReadings = 0
        XCTAssertEqual(
            CacheSpy.numberOfCacheReadings,
            expectedNumberOfCacheReadings,
            """
            Expected the cache not to be read, but got \(CacheSpy.numberOfCacheReadings) cache
            readings instead.
            """
        )
        ...
        XCTAssertEqual(
            ...
            expectedNumberOfCacheWritings,
            """
            Expected no data to be cached, but number of cache writings =
            \(CacheSpy.numberOfCacheReadings) instead.
            """
        )
        ...
        case .success(let platformImage):
            XCTFail(
                """
                Expected attempting to load data from a `nil` URL to return an error,
                but got \(platformImage) instead.
                """
            )
        case .failure(let error):
            if case .emptyURL = error {
                XCTAssertTrue(true)
            } else {
                XCTFail(
                    """
                    Expected error thrown when attempting to load data from invalid URL to be
                    `.emptyURL`, but got \(String(describing: error)) instead).
                    """
                )
            }
        case nil:
            XCTFail(
                """
                Expected to receive an `AsyncImageCachingLoaderError` when attempting to
                load data from a `nil` URL, but got `nil` instead.
                """
            )
        }
    }
    ...
    func test_load_invalidURL_doesNotCacheAndReturnsExpectedError() async throws {
        ...
        case .success(let platformImage):
            XCTFail(
                """
                Expected attempting to load data from an invalid URL to return an error,
                but got \(platformImage) instead.
                """
            )
        case .failure(let error):
        ...
        case nil:
            XCTFail(
                """
                Expected to receive an `AsyncImageCachingLoaderError` when attempting to
                load data from and invalid URL, but got `nil` instead.
                """
            )
        }
    }
